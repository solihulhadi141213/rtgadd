<?php
    ...
    // Validasi File
    if(empty($_FILES['data_instansi']['name'])) {
        echo json_encode([
            'status' => 'error',
    ...
    $nama_file = $_FILES['data_instansi']['name'];
    $file_mimes = array(
    ...
    if(isset($_FILES['data_instansi']['name']) && in_array($_FILES['data_instansi']['type'], $file_mimes)) {
        $arr_file = explode('.', $_FILES['data_instansi']['name']);
        ...
        try {
            $spreadsheet = $reader->load($_FILES['data_instansi']['tmp_name']);
