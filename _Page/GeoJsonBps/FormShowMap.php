<?php
    ...
    if (!$Qry->execute()) {
        ...
        $coordinates    = showOrDash($Data['coordinates']);

        // Validasi apakah coordinates adalah JSON yang valid
        $isValidGeoJson = false;
        if ($coordinates !== "-") {
            $geoJsonData = json_decode($coordinates, true);
            $isValidGeoJson = (json_last_error() === JSON_ERROR_NONE);
        }
        ...
        echo '
            ...
            <div class="row mb-2">
                <div class="col-4"><small>Koordinat</small></div>
                <div class="col-1"><small>:</small></div>
                <div class="col-7">
                    <small class="text text-grayish">
                        <pre id="geoJsonData" style="display:none;">'.htmlspecialchars($coordinates).'</pre>
                        '.($isValidGeoJson ? '<span class="badge bg-success">GeoJSON Valid</span>' : '<span class="badge bg-danger">Format GeoJSON Tidak Valid</span>').'
                    </small>
                </div>
            </div>
            <div class="row mb-2">
                <div class="col-12">
                    <label for="geo_json_example">
                        <small>Geo JSON</small>
                    </label>
                   <textarea class="form-control mb-3" id="geo_json_example" style="width:100%; height:400px;">'.$coordinates.'</textarea>
                </div>
            </div>
        ';
    }
